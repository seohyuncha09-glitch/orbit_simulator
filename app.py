...
with col1:
    ...
    # 파이썬 f-string 포맷팅 에러를 방지하기 위해 중괄호 대입 문제를 완벽히 우회
    html_template = """
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <style>
            ...
            .uiBtn { background: #111111; border: 1px solid #555555; color: white; padding: 6px 14px; font-size: 13px; font-weight: bold; border-radius: 4px; cursor: pointer; transition: all 0.2s; }
    ...
    <body>
        <div id="container">
            <div id="infoOverlay">
                <div id="timeText">Time: 0.0 / VAR_T days</div>
        ...
        <script>
            ...
            const T = VAR_T;
            const mu = VAR_MU;
            const starColor = "VAR_STAR_COLOR";
            
            const showEarthOrbit = VAR_SHOW_EARTH;
            const showHabitableZone = VAR_SHOW_HZ;
            const highlightPlanet = VAR_HIGHLIGHT;
            
            const hzInner = VAR_HZ_INNER;
            const hzOuter = VAR_HZ_OUTER;
            const starRadAU = VAR_STAR_RAD * SOLAR_RAD_TO_AU;
            const plRade = VAR_PL_RADE;
            ...
            zoomSlider.addEventListener('input', (event) => {
                ...
                let displayTxt = currentZoom >= 1000 ? (currentZoom/1000).toFixed(1) + "k x" : currentZoom.toFixed(1) + "x";
    ...
    # 안전하게 문자열 치환(Replace) 방식으로 데이터 주입 (파이썬 중괄호 문법 충돌 원천 차단)
    html_code = html_template\
        ...
        .replace("VAR_C", str(c))\
        .replace("VAR_T", str(T))\
        .replace("VAR_MU", str(mu))\
        .replace("VAR_STAR_COLOR", str(star_color))\
        .replace("VAR_SHOW_EARTH", str(show_earth_orbit).lower())\
        .replace("VAR_SHOW_HZ", str(show_habitable_zone).lower())\
        .replace("VAR_HIGHLIGHT", str(highlight_planet).lower())\
        .replace("VAR_HZ_INNER", str(hz_inner))\
        .replace("VAR_HZ_OUTER", str(hz_outer))\
        .replace("VAR_STAR_RAD", str(star_rad))\
        .replace("VAR_PL_RADE", str(pl_rade))

    st.components.v1.html(html_code, height=660, key=f"orbit_sim_safe_{selected_planet}")
